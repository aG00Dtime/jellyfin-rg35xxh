...
Copy-Item -LiteralPath "port/port.json" -Destination (Join-Path $stage "port.json")
Copy-Item -LiteralPath "port/README.md" -Destination (Join-Path $stage "README.md")
Copy-Item -LiteralPath "port/gameinfo.xml" -Destination (Join-Path $stage "gameinfo.xml")
...
$required = @('jellyfinrg35xx/jellyfin_rg35xx.py','jellyfinrg35xx/playback.py','jellyfinrg35xx/library.py','jellyfinrg35xx/ui.py','jellyfinrg35xx/config.example.json','jellyfinrg35xx/assets/jellyfin-icon.png','jellyfinrg35xx/cover.png','gameinfo.xml','Jellyfin RG35XX.sh','port.json','README.md')
foreach ($item in $required) { if ($entries -notcontains $item) { throw "Invalid package: missing $item" } }
$manifest = Get-Content -LiteralPath (Join-Path $stage 'port.json') -Raw | ConvertFrom-Json
